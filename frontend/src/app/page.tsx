...
import { AccountCircle, ThumbUp, ThumbDown } from '@mui/icons-material';
...
export default function Home() {
  ...
  const [cards, setCards] = useState([
    {
      ...
      description:
        'Rejoignez une équipe créative et réalisez des projets uniques dans l’art digital.',
    },
    {
      id: 2,
  ...
  const swipeLeft = (cardId: number) => {
    controls
      .start({ x: -window.innerWidth, opacity: 0 })
      .then(() => handleSwipe('left', cardId));
  };

  const swipeRight = (cardId: number) => {
    controls
      .start({ x: window.innerWidth, opacity: 0 })
      .then(() => handleSwipe('right', cardId));
  };
  ...
  return (
    ...
    >
      ...
      <Typography
        variant="h4"
        color="primary"
        sx={{ marginTop: '40px', fontWeight: 'bold', color: '#333' }}
      >
        ...
        {cards.length > 0 ? (
          <>
            <motion.div
              ...
              onDragEnd={(e, { offset }) => {
                const swipe =
                  offset.x > 200 ? 'right' : offset.x < -200 ? 'left' : 'hold';
                if (swipe === 'right') {
            ...
            >
              <Card
                sx={{
                  ...
                  filter:
                    swipeDirection === 'left'
                      ? 'brightness(0.7) sepia(1) hue-rotate(-50deg)'
                      : swipeDirection === 'right'
                      ? 'brightness(0.7) sepia(1) hue-rotate(90deg)'
                      : 'none',
              ...
              >
                ...
                >
                  ...
                  <Typography variant="subtitle1">{cards[0].company}</Typography>
            ...
            {/* Boutons pour swiper à gauche ou à droite */}
            <Box
              sx={{
                display: 'flex',
